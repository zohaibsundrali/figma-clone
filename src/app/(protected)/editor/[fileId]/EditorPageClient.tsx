  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-[#1e1e1e]">
      ...
      <div className="flex h-12 items-center gap-3 border-b border-[#444444] bg-[#2c2c2c] px-3 flex-shrink-0">
        <div className="h-4 w-24 rounded bg-[#383838] animate-pulse" />
        <div className="h-4 w-px bg-[#444444]" />
        <div className="h-4 w-32 rounded bg-[#383838] animate-pulse" />
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Left panel skeleton */}
        <div className="w-52 border-r border-[#444444] bg-[#2c2c2c] p-3 flex flex-col gap-3 flex-shrink-0">
          <div className="h-6 w-3/4 rounded bg-[#383838] animate-pulse" />
          <div className="space-y-2">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="h-8 w-full rounded-md bg-[#383838] animate-pulse" style={{ animationDelay: `${i * 50}ms` }} />
            ))}
          </div>
        </div>
        ...
        <div className="relative flex-1 bg-[#1e1e1e] flex items-center justify-center">
          <div className="text-center space-y-3">
            ...
            <p className="text-sm text-[#b3b3b3] font-medium">Loading canvas…</p>
          </div>
          {/* Faint grid overlay */}
        ...
        <div className="w-72 border-l border-[#444444] bg-[#2c2c2c] flex flex-col flex-shrink-0">
          ...
          <div className="flex border-b border-[#444444] p-1 gap-1">
            {["Design", "Proto", "Inspect"].map((t) => (
              <div key={t} className="h-7 flex-1 rounded bg-[#383838] animate-pulse" />
            ))}
          </div>
          {/* Panel content */}
          <div className="flex-1 p-3 space-y-4">
            {[...Array(4)].map((_, g) => (
              <div key={g} className="space-y-2">
                <div className="h-3 w-20 rounded bg-[#383838] animate-pulse" />
                <div className="grid grid-cols-2 gap-2">
                  {[...Array(4)].map((_, i) => (
                    <div key={i} className="h-8 rounded bg-[#383838] animate-pulse" style={{ animationDelay: `${(g * 4 + i) * 30}ms` }} />
                  ))}
                </div>
              </div>
            ))}
